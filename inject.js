...
function defineVideoController() {
  ...
  tc.videoController.prototype.initializeControls = function () {
    ...
    const speed = this.video.playbackRate.toFixed(3);
...
function setupListener() {
  ...
  function updateSpeedFromEvent(video) {
    ...
    var speed = Number(video.playbackRate.toFixed(3));

    log("Playback rate changed to " + speed, 4);

    log("Updating controller with new speed", 5);
    speedIndicator.textContent = speed.toFixed(3);
...
function setMark1(v) {
  log("Adding marker1", 5);
  v.vsc.mark1 = v.currentTime;
}
function setMark2(v) {
...
function jumpToMark1(v) {
  log("Recalling Marker1", 5);
  if (v.vsc.mark1 && typeof v.vsc.mark1 === "number") {
    v.currentTime = v.vsc.mark1;
  }
}
